...
function obtenerUsuariosDeLaBaseDeDatos($id_usuario) {
    ...
    if ($result->num_rows > 0) {
        ...
        echo "No se encontraron otros usuarios";
        exit;
    }
}
...
<?php $i = 0; foreach ($usuarios as $usuario): ?>
    <div class="usuario" id="usuario-<?php echo $i; ?>" style="<?php echo $i > 0 ? 'display: none;' : ''; ?>">
        ...
        <img class="foto-usuario" src="imagen.php?id=<?php echo $usuario['id_us']; ?>">
        <a href="perfil_usuario.php?id_us=<?php echo $usuario['id_us']; ?>">Ver perfil</a>
...
document.getElementById('like').addEventListener('click', cambiarUsuario);
document.getElementById('hate').addEventListener('click', cambiarUsuario);

function cambiarUsuario() {
    // Ocultar el usuario actual
    document.getElementById('usuario-' + usuarioActual).style.display = 'none';

    // Incrementar el usuario actual
    usuarioActual++;

    // Si hemos pasado por todos los usuarios, volver al principio
    if (usuarioActual >= totalUsuarios) {
        usuarioActual = 0;
    }

    // Mostrar el nuevo usuario actual
